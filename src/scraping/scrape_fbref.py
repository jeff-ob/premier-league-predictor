#!/usr/bin/env python3
"""
CLI pour scraper les données xG depuis FBref.

Usage:
    python -m src.scraping.scrape_fbref --seasons 2023-2024
    python -m src.scraping.scrape_fbref --seasons 2017-2026 --delay 2.0 --force-refresh
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Scrape xG data from FBref for Premier League matches',
...
def display_confirmation(start_year: int, end_year: int, delay: float, force_refresh: bool):
    ...
    print("\n" + "="*70)
    print("FBref xG Scraper - Configuration")
    print("="*70)
...
def main():
    ...
    try:
        ...
        print("-" * 70)
        print(f"\nScraping complete! Collected {len(df)} matches.")
        
        # Export to CSV
        print("\nExporting to CSV...")
        csv_path = exporter.export_fbref(
            df=df,
        ...
        report_path = f"data/external/validation_report_fbref_{timestamp}.txt"
